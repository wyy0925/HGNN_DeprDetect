...
from Model_Base import EEGEncoder,Pool_Generator,GCNN,AdjGenerator,normalize_A
...
if __name__ == '__main__':
    ...
    model = HybGNN_fix(input_sample=1000).cuda()
    out = model(data)
    # for HUSM
    data_ = torch.rand((128, 19, 1024)).cuda()
    model_ = HybGNN_fix(input_sample=1024).cuda()
    out_ = model_(data_)